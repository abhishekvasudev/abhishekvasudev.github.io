...
export default function FeaturedProjectDetailPage({ content }: FeaturedProjectDetailPageProps) {
  ...
  if (!project) {
    return (
      <>
        <SEO
          title="Project Not Found | Abhishek Vasudev"
  ...
  const projectUrl = `/featured-projects/${project.slug}`;
  const ogImage = project.image ? absoluteUrl(project.image) : undefined;
  ...
  return (
    <>
      ...
      <article className="pt-36 pb-20 px-4 md:px-8 max-w-4xl mx-auto">
        <Link
          to="/"
        ...
        <div className="glass-panel rounded-2xl border border-white/5 p-8 md:p-10">
          {project.image && (
